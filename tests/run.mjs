...
const { SessionSelectorComponent: Picker } = await jiti.import(join(plugin, 'session-selector.ts'));
const search = await jiti.import(join(plugin, 'session-selector-search.ts'));
const nativeSearch = await importFile(join(piDir, 'dist/modes/interactive/components/session-selector-search.js'));
const config = await jiti.import(join(plugin, 'config.ts'));
const launcher = await jiti.import(join(plugin, 'terminal-launcher.ts'));
const paths = await jiti.import(join(plugin, 'paths.ts'));
const registry = await jiti.import(join(plugin, 'active-sessions.ts'));
